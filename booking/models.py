...
class BookPost(models.Model):
    ...
    accommodation_type = models.CharField(
        max_length=10, choices=ACCOMMODATION_TYPES
    )
    ...
    def image_tag(self):
        """Create custom view in admin panel displaying image preview"""
        if "placeholder" in self.booking_image.url:
            placeholder_url = static("images/default.png")
            return mark_safe(
                f'<img src="{placeholder_url}" width="150" height="150" />'
            )
        return mark_safe(
            f'<img src="{self.booking_image.url}" width="150" height="150" />'
        )
